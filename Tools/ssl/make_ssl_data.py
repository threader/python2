...
def gen_library_codes(args):
    ...
    for libname in sorted(args.lib2errnum):
        yield "#ifdef ERR_LIB_{}".format(libname)
        yield '    {{"{}", ERR_LIB_{}}},'.format(libname, libname)
        yield "#endif"
    yield "    { NULL }"
    yield "};"
    yield ""
...
def gen_error_codes(args):
    ...
    for reason, libname, errname, num in args.reasons:
        yield "  #ifdef {}".format(reason)
        yield '    {{"{}", ERR_LIB_{}, {}}},'.format(errname, libname, reason)
        yield "  #else"
        yield '    {{"{}", {}, {}}},'.format(errname, args.lib2errnum[libname], num)
        yield "  #endif"
...
def main():
    ...
    if not os.path.isfile(args.errtxt):
        parser.error("File {} not found in srcdir\n.".format(args.errtxt))
    ...
    lines = [
        ...
        "/* Generated on {} */".format(datetime.datetime.utcnow().isoformat())
    ]
